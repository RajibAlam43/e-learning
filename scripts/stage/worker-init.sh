...
deploy ALL=(root) NOPASSWD: /usr/bin/systemctl restart e-learning-worker, /usr/bin/systemctl is-active --quiet e-learning-worker, /usr/bin/systemctl status e-learning-worker --no-pager, /usr/bin/journalctl -u e-learning-worker -n 150 --no-pager
EOF
