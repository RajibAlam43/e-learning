...
CREATE TABLE verification_codes (
                                    ...
                                    CONSTRAINT chk_verification_codes_purpose
                                        CHECK (purpose IN ('VERIFY_PHONE', 'PASSWORD_RESET')),

                                    CONSTRAINT chk_verification_codes_attempt_count_non_negative
                                        CHECK (attempt_count >= 0),

                                    CONSTRAINT chk_verification_codes_max_attempts_positive
                                        CHECK (max_attempts > 0),

                                    CONSTRAINT chk_verification_codes_sent_count_positive
                                        CHECK (sent_count > 0),

                                    CONSTRAINT chk_verification_codes_used_or_revoked
                                        CHECK (used_at IS NULL OR revoked_at IS NULL)
);

CREATE INDEX idx_verification_codes_user
    ON verification_codes(user_id);

CREATE INDEX idx_verification_codes_purpose
    ON verification_codes(purpose);

CREATE INDEX idx_verification_codes_expires_at
    ON verification_codes(expires_at);

CREATE INDEX idx_verification_codes_user_purpose_active
