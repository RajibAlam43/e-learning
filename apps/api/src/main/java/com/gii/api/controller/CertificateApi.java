...
public interface CertificateApi {
    ...
    @Operation(
            ...
            description = "Check eligibility and issue certificate if qualified; return existing if already issued. Idempotent operation.",
            security = @SecurityRequirement(name = "bearerAuth")
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Certificate issued or retrieved", content = @Content(schema = @Schema(implementation = CertificateIssueResponse.class))),
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Download URL retrieved", content = @Content(schema = @Schema(implementation = CertificateDownloadUrlResponse.class))),
            @ApiResponse(responseCode = "401", description = "Unauthorized"),
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Verification result retrieved", content = @Content(schema = @Schema(implementation = PublicCertificateVerificationResponse.class))),
            @ApiResponse(responseCode = "404", description = "Certificate not found")
    })
